...
Le patch **Meteo Consult: remove advertisements** neutralise l'inventaire renvoyé
aux vues publicitaires. Les bannières, vidéos et interstitiels passent ainsi par
...
Le patch **Meteo Consult: disable subscription promotions** désactive les
sollicitations automatiques du paywall. Il force le drapeau de configuration de la
popup à `false` et bloque la décision finale qui programme l'interstitiel de
promotion. Les écrans de compte et d'abonnement ouverts volontairement restent
accessibles. Comme pour le patch de fin de prévisions, aucune donnée supplémentaire
n'est créée lorsque l'API ne renvoie pas les heures au-delà de 24 heures.
...
5. Avec **Meteo Consult: remove advertisements**, vérifier qu'aucune bannière,
   vidéo ou interstitiel publicitaire ne s'affiche dans les écrans habituels.
6. Avec **Meteo Consult: disable subscription promotions**, attendre les écrans
   d'accueil et de prévisions ; aucune sollicitation automatique ne doit apparaître.
7. Vérifier que l'accès volontaire à l'abonnement depuis le compte fonctionne toujours.

Les trois patches Meteo Consult peuvent être sélectionnés ensemble. Ils restent
indépendants dans Morphe pour faciliter le diagnostic pendant la pré-release.
